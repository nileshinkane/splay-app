...
exports.getFeaturedVideos = (req, res) => {
  Video.find({ departmentFeatured: true }, function (err, videos) {
    if (err) {
      res.status(400).json({
        error: err,
      });
...
exports.getVideosData = (req, res) => {
  Video.find({}, function (err, videos) {
    var deptData = {};
    videos.forEach(function (v) {
      deptData[v.department] = (deptData[v.department] || 0) + 1;
    });
    res.json({ total: videos.length, data: deptData });
  });
};
