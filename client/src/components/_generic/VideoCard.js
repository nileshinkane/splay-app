...
const VideoCard = (props) => {
  ...
  let smallScreenLetter = window.innerWidth < 700 ? 15 : 45;

  checkedTitle =
    props.title &&
    (props.title.split(" ").length >= 15
      ? props.title.split(" ").slice(0, 15).join(" ").concat("...")
      : props.title);
  checkedDescription =
    props.description &&
    (props.description.split(" ").length >= 45
      ? props.description
          .split(" ")
          .slice(0, smallScreenLetter)
          .join(" ")
          .concat("...")
      : props.description);
    ...
    return (
      <div className={classes.rootLong}>
        <div className={classes.leftLong}>
          ...
          >
            <img
              src={props.thumbnail || fakeImg}
              className={classes.imgLong}
              alt="thumbnail"
            />
        ...
        <div className={classes.rightLong}>
          ...
          <p style={{ margin: "3% 0 0 0" }}>
            <span className={classes.bold}>CSE &amp; IT</span>
          </p>
  ...
  return (
    <div className={classes.root} style={props.style}>
      ...
      <div className={classes.footer}>
        <Typography style={{ padding: "8px 10px" }} variant="subtitle1" noWrap>
          {props.title || "This is a temporary title for a temporary video"}
        </Typography>
        <Typography
          style={{ color: "gray", padding: "0 10px" }}
          variant="subtitle2"
          noWrap
        >
          {props.postedBy ? `By ${props.postedBy}` : "By Admin"}
        </Typography>
      </div>
    </div>
  );
