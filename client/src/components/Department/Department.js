...
function Department(props) {
  ...
  useEffect(() => {
    fetch(
      `${process.env.REACT_APP_API_URL}/dept/${props.match.params.deptName}`,
      {
        method: "GET",
  ...
  return (
    <SideComponent>
      {state ? (
        <DeptName intro={state.dept.description} {...props} />
      ) : (
        <DeptNameSkeleton />
      )}
      {state ? (
        <DeptFeatured videos={state} {...props} />
      ) : (
        <DeptFeaturedSkeleton />
      )}
      <DeptAbout {...props} />
    </SideComponent>
  );
